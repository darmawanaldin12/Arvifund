...
export default function InputModal({ onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    // iOS Safari supports webkitSpeechRecognition but it's buggy/requires user gesture per session
    // We still try to set it up but handle errors gracefully
    try {
      const rec = new SpeechRecognition()
      rec.continuous = !ios // iOS doesn't support continuous well
      ...
      console.error('SpeechRecognition setup failed:', err)
      setVoiceSupported(false)
    }
  ...
  function toggleRecording() {
    if (!recognition || !voiceSupported) {
      if (iosDevice) {
        setError('Voice note di iPhone: Pastikan menggunakan Safari terbaru (iOS 14.5+) dan izinkan akses mikrofon saat diminta.')
      } else {
        setError('Browser Anda tidak mendukung perekaman suara. Coba gunakan Chrome atau Safari terbaru.')
      }
      return
    }
    ...
    if (isRecording) {
      ...
      try {
        ...
        console.error(err)
        // On iOS, sometimes we need to recreate the recognition instance
        if (iosDevice) {
          setError('Tekan tombol suara sekali lagi. Safari membutuhkan interaksi langsung untuk memulai mikrofon.')
        } else {
          setError('Gagal memulai perekaman: ' + err.message)
        }
        setIsRecording(false)
      }
    }
        ...
        <div className="modal-content">
          ...
          {mode === 'ai' ? (
            <div>
              ...
              {/* iOS voice note info banner */}
              {iosDevice && (
                <div style={{
                  padding: '10px 12px', marginBottom: 12,
                  background: 'rgba(56,189,248,0.08)',
                  border: '1px solid rgba(56,189,248,0.25)',
                  borderRadius: 8, fontSize: 12, color: 'var(--text2)', lineHeight: 1.5,
                }}>
                  📱 <strong>iPhone tip:</strong> Gunakan tombol 🎙️ di keyboard untuk dikte langsung ke kolom teks di atas, atau tekan tombol Suara di bawah. Pastikan Safari versi terbaru.
                </div>
              )}
            ...
            <>
              ...
              <form onSubmit={handleSubmit}>
                <div className="form-group">
                  ...
                  <input className="form-input ios-input" type="date" value={form.tanggal} onChange={e => set('tanggal', e.target.value)} required />
                </div>
                <div className="form-group">
                  <label className="form-label">{tipe === 'income' ? 'Sumber' : tipe === 'cash' ? 'Lokasi ATM' : 'Toko / Merchant'}</label>
                  <input className="form-input ios-input" type="text" placeholder={tipe === 'income' ? 'Nama perusahaan' : tipe === 'cash' ? 'Nama ATM / lokasi' : 'Nama toko'} value={form.toko} onChange={e => set('toko', e.target.value)} />
                </div>
                <div className="form-group">
                  <label className="form-label">{tipe === 'income' ? 'Keterangan' : 'Uraian / Items'}</label>
                  <input className="form-input ios-input" type="text" placeholder="Deskripsi transaksi" value={form.uraian} onChange={e => set('uraian', e.target.value)} />
                </div>
                <div className="form-group">
                  <label className="form-label">{tipe === 'income' ? 'Jumlah' : 'Total'}</label>
                  <input className="form-input ios-input" type="number" inputMode="numeric" placeholder="0" value={form.total} onChange={e => set('total', e.target.value)} required min="0" />
                </div>
                {tipe === 'expense' && (
                  <div className="form-group">
                    <label className="form-label">Kategori</label>
                    <select className="form-select ios-input" value={form.kategori} onChange={e => set('kategori', e.target.value)}>
                ...
                {tipe !== 'cash' && (
                  <div className="form-group">
                    ...
                    <select className="form-select ios-input" value={form.metode} onChange={e => set('metode', e.target.value)}>
                ...
                <div className="form-group">
                  ...
                  <select className="form-select ios-input" value={form.bank} onChange={e => set('bank', e.target.value)}>
                    {BANK_LIST.map(b => <option key={b}>{b}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label className="form-label">User</label>
                  <select className="form-select ios-input" value={form.user_id} onChange={e => set('user_id', e.target.value)} required>
                    <option value="">Pilih User</option>
