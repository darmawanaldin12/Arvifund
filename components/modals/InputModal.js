  ...
  const loadingOverlay = aiLoading && !showConfirm ? (
    <div style={{
      ...
      {imagePreview ? (
        ...
        /* ── AI THINKING ANIMATION (voice / teks) ── */
        <div style={{
          background: 'var(--surface)', borderRadius: 24,
          padding: '32px 28px', width: 260,
          boxShadow: '0 16px 48px rgba(0,0,0,0.4)',
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
              {[0, 0.15, 0.3].map((delay, i) => (
                <span key={i} style={{ display: 'inline-block', width: 7, height: 7, borderRadius: '50%', background: 'var(--accent)', animation: `dotPulse 1.4s ease-in-out ${delay}s infinite` }} />
              ))}
            </div>
          </div>
