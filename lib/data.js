...
// ── EXPENSES ───────────────────────────────────────────────
// Untuk tampilan UI (Overview, list transaksi) — tanpa [AUDIT]
...
// Untuk hitung saldo rekening — semua data termasuk [AUDIT]
export async function getAllExpensesForBalance() {
  const { data, error } = await supabase
    .from('expenses')
    .select('*')
    .order('tanggal', { ascending: false })
...
// ── INCOME ─────────────────────────────────────────────────
// Untuk tampilan UI (Overview, list income) — tanpa [AUDIT]
...
// Untuk hitung saldo rekening — semua data termasuk [AUDIT]
export async function getAllIncomeForBalance() {
  const { data, error } = await supabase
    .from('income')
    .select('*')
    .order('tanggal', { ascending: false })
...
// expenses/income = untuk tampilan UI (tanpa AUDIT)
// allExpenses/allIncome = untuk hitung saldo bankBalances (termasuk AUDIT)
export async function getAllDashboardData() {
  const [expenses, income, allExpenses, allIncome, cashRecords, budgetPlans, profiles, transfers] = await Promise.all([
    getExpenses(),
    getIncome(),
    getAllExpensesForBalance(),
    getAllIncomeForBalance(),
    getCashRecords(),
    getBudgetPlans(),
    getAllProfiles(),
    getTransfers(),
  ])
  return { expenses, income, allExpenses, allIncome, cashRecords, budgetPlans, profiles, transfers }
}
