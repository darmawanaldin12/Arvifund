...
// ── BUILD PERIODS (dengan support override per bulan) ───────
// URUTAN BARU: index 0 = periode TERBARU/berjalan, index 11 = terlama.
// Filter bar akan menampilkan bulan berjalan di posisi paling kiri (pertama),
// geser kanan = bulan-bulan sebelumnya.
export function getEffectivePayDate(bulanNama, tahun, payPeriodDate = 25, overrides = {}) {
  const key = `${bulanNama}-${tahun}`
  return overrides[key] || payPeriodDate
}

export function buildPeriods(payPeriodDate = 25, overrides = {}) {
  const today = new Date()
  const periods = []
  // i = 0  → bulan berjalan (paling baru, index 0 di array)
  // i = 11 → 11 bulan lalu (paling lama, index 11 di array)
  for (let i = 0; i <= 11; i++) {
    const startRef = new Date(today.getFullYear(), today.getMonth() - i, 1)
    const sY = startRef.getFullYear()
    const sM = startRef.getMonth()
    const startBulanNama = BULAN_ORDER[sM]
    const startTgl = getEffectivePayDate(startBulanNama, sY, payPeriodDate, overrides)
    const startDate = new Date(sY, sM, startTgl)
    startDate.setHours(0, 0, 0, 0)
    const endRef = new Date(sY, sM + 1, 1)
    const eY = endRef.getFullYear()
    const eM = endRef.getMonth()
    const endBulanNama = BULAN_ORDER[eM]
    const endTgl = getEffectivePayDate(endBulanNama, eY, payPeriodDate, overrides)
    const endDate = new Date(eY, eM, endTgl - 1)
    endDate.setHours(23, 59, 59, 999)
    const startLabel = BULAN_ORDER[startDate.getMonth()]
    const endLabel   = BULAN_ORDER[endDate.getMonth()]
    const label = startTgl + ' ' + startLabel + ' – ' + (endTgl - 1) + ' ' + endLabel
    periods.push({ start: startDate, end: endDate, label, index: i, startTgl, endTgl })
  }
  return periods
}

// Cari index periode yang sedang berjalan (biasanya 0 karena index 0 = berjalan)
export function getCurrentPeriodIndex(payPeriodDate = 25, overrides = {}) {
...
export function filterByPeriod(rows, periodIdx, payPeriodDate = 25, overrides = {}) {
...
export const KATEGORI_LIST = [
  ...
  'Hiburan','Tagihan','Cicilan','Investasi','Bisnis','Biaya Admin','Lainnya','Pemasukan'
]
...
// Warna badge per metode bayar (dipakai di tab "Semua" transaksi)
export const METODE_COLOR = {
  'Cash': '#16a34a',
  'Transfer': '#3b82f6',
  'QRIS': '#8b5cf6',
  'Card': '#f97316',
  'Cardless': '#06b6d4',
  'Virtual Account Transfer': '#6366f1',
}
