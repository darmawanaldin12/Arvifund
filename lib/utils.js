...
export function fmt(val) {
  ...
  const n = Number(val)
  if (Math.abs(n) >= 1_000_000_000) return 'Rp ' + (n / 1_000_000_000).toFixed(3).replace('.', ',') + 'M'
  if (Math.abs(n) >= 1_000_000)     return 'Rp ' + (n / 1_000_000).toFixed(3).replace('.', ',') + 'jt'
...
// ── BUILD PERIODS ──────────────────────────────────────────
// overrides: { 'Mei-2026': 25, 'April-2026': 24, ... } — tanggal gajian per bulan
// payPeriodStart: string 'YYYY-MM-DD' — tanggal mulai pertama kali (opsional)
export function buildPeriods(payPeriodDate = 25, overrides = {}, payPeriodStart = null) {
  const today = new Date()
  const periods = []

  // Helper: ambil tanggal gajian untuk bulan tertentu (cek overrides dulu)
  function getTgl(bulanName, tahun) {
    const key = `${bulanName}-${tahun}`
    return (overrides && overrides[key] != null) ? overrides[key] : payPeriodDate
  }

  let firstStart
  if (payPeriodStart) {
    const ps = new Date(payPeriodStart)
    const bulanName = BULAN_ORDER[ps.getMonth()]
    const tgl = getTgl(bulanName, ps.getFullYear())
    firstStart = new Date(ps.getFullYear(), ps.getMonth(), tgl)
  } else {
    // Fallback: 12 bulan ke belakang dari periode aktif
    let currentStart
    if (today.getDate() >= payPeriodDate) {
      currentStart = new Date(today.getFullYear(), today.getMonth(), payPeriodDate)
    } else {
      currentStart = new Date(today.getFullYear(), today.getMonth() - 1, payPeriodDate)
    }
    firstStart = new Date(currentStart.getFullYear(), currentStart.getMonth() - 11, payPeriodDate)
  }

  // Generate dari firstStart sampai melewati hari ini
  let cursor = new Date(firstStart)
  while (cursor <= today) {
    const startBulanName = BULAN_ORDER[cursor.getMonth()]
    const startTgl = getTgl(startBulanName, cursor.getFullYear())

    // startDate pakai tanggal gajian bulan ini
    const startDate = new Date(cursor.getFullYear(), cursor.getMonth(), startTgl)

    // endDate: tanggal gajian bulan berikutnya - 1
    const nextMonth = new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1)
    const nextBulanName = BULAN_ORDER[nextMonth.getMonth()]
    const nextTgl = getTgl(nextBulanName, nextMonth.getFullYear())
    const endDate = new Date(nextMonth.getFullYear(), nextMonth.getMonth(), nextTgl - 1)
    endDate.setHours(23, 59, 59, 999)

    const startBulan = BULAN_ORDER[startDate.getMonth()]
    const endBulan   = BULAN_ORDER[endDate.getMonth()]
    const startStr   = `${startTgl} ${startBulan} ${startDate.getFullYear()}`
    const endStr     = `${nextTgl - 1} ${endBulan} ${endDate.getFullYear()}`
    const label      = `${startStr} – ${endStr}`

    periods.push({ start: startDate, end: endDate, label, startDate: startStr, endDate: endStr })
...
export function getCurrentPeriodIndex(payPeriodDate = 25, overrides = {}, payPeriodStart = null) {
  const periods = buildPeriods(payPeriodDate, overrides, payPeriodStart)
  const now = new Date()
...
export function filterByPeriod(rows, periodIdx, payPeriodDate = 25, overrides = {}, payPeriodStart = null) {
