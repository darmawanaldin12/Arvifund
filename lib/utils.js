...
export function buildPeriods(payPeriodDate = 25) {
  ...
  // Tentukan awal periode aktif
  // Jika hari ini >= payPeriodDate → periode mulai bulan ini tgl payPeriodDate
  // Jika hari ini < payPeriodDate  → periode mulai bulan lalu tgl payPeriodDate
  let currentStart
  ...
  // Generate 12 periode ke belakang; index 11 = periode aktif (paling kanan)
  for (let i = 11; i >= 0; i--) {
    const startDate = new Date(currentStart.getFullYear(), currentStart.getMonth() - i, payPeriodDate)
    ...
    const startStr   = `${payPeriodDate} ${startBulan}`
    const endStr     = `${payPeriodDate - 1} ${endBulan}`
    const label      = `${startStr} – ${endStr}`

    periods.push({ start: startDate, end: endDate, label, startDate: startStr, endDate: endStr, index: 11 - i })
  }
  return periods
}
