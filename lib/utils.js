...
// Kalau ada payPeriodStart, generate dari situ sampai sekarang
// Kalau tidak ada, fallback ke logika lama (12 periode mundur dari sekarang)
export function buildPeriods(payPeriodDate = 25, overrides = {}, payPeriodStart = null) {
  ...
  let firstStart
  if (payPeriodStart) {
    // Parse payPeriodStart: ambil tgl dari string, paksa pakai payPeriodDate
    const ps = new Date(payPeriodStart)
    firstStart = new Date(ps.getFullYear(), ps.getMonth(), payPeriodDate)
  } else {
  ...
  while (cursor <= today) {
    ...
    const endDate   = new Date(startDate.getFullYear(), startDate.getMonth() + 1, payPeriodDate - 1)
    endDate.setHours(23, 59, 59, 999)

    const startBulan = BULAN_ORDER[startDate.getMonth()]
    const endBulan   = BULAN_ORDER[endDate.getMonth()]
    const startStr   = `${payPeriodDate} ${startBulan} ${startDate.getFullYear()}`
    const endStr     = `${payPeriodDate - 1} ${endBulan} ${endDate.getFullYear()}`
    const label      = `${startStr} – ${endStr}`

    periods.push({ start: startDate, end: endDate, label, startDate: startStr, endDate: endStr })

    cursor = new Date(cursor.getFullYear(), cursor.getMonth() + 1, payPeriodDate)
  }
