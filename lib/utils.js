...
export function buildPeriods(payPeriodDate = 25, overrides = {}) {
  ...
  const periods = []
  for (let i = 11; i >= 0; i--) {
    const refMonth = new Date(today.getFullYear(), today.getMonth() - i, 1)
    const y  = refMonth.getFullYear()
    const m  = refMonth.getMonth()
    const startBulanNama = BULAN_ORDER[m === 0 ? 11 : m - 1]
    const startTahun     = m === 0 ? y - 1 : y
    const startTgl       = getEffectivePayDate(startBulanNama, startTahun, payPeriodDate, overrides)
    const endBulanNama   = BULAN_ORDER[m]
    const endTgl         = getEffectivePayDate(endBulanNama, y, payPeriodDate, overrides)
    const startDate = new Date(startTahun, m === 0 ? 11 : m - 1, startTgl)
    const endDate   = new Date(y, m, endTgl - 1)
    endDate.setHours(23, 59, 59, 999)
    const startLabel = BULAN_ORDER[startDate.getMonth()]
    const endLabel   = BULAN_ORDER[endDate.getMonth()]
    const sameMonth  = startDate.getMonth() === endDate.getMonth() && startDate.getFullYear() === endDate.getFullYear()
    const label = sameMonth
      ? `${startLabel} ${startDate.getFullYear()}`
      : `${startTgl} ${startLabel} – ${endTgl - 1} ${endLabel}`
