...
// ── Cek support ──────────────────────────────────────────────
// iOS Safari kadang return false saat dipanggil terlalu cepat.
// Kita coba sampai 3x dengan jeda singkat.
export async function isBiometricSupported() {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (ok) return true
      // Jika false, tunggu sebentar sebelum retry (hanya iOS butuh ini)
    ...
    assertion = await navigator.credentials.get({
      publicKey: {
        ...
        allowCredentials: [{
          ...
          // iOS butuh 'internal', Android butuh 'internal' juga.
          // Jangan tambah 'hybrid' karena bisa trigger passkey picker asing.
          transports: ['internal'],
        }],
        userVerification: 'required',
        timeout: 60000,
