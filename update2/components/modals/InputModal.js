...
export default function InputModal({ onClose, onSuccess }) {
  ...
  const SILENCE_DELAY  = 2000 // 2s silence → auto stop
            ...
            <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 14px', borderBottom: i < arr.length - 1 ? '1px solid var(--border)' : 'none' }}>
              ...
              {row.isUserSelect ? (
                <select
                  ...
                  style={{ background: 'var(--surface)', color: 'var(--text1)', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 8px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', outline: 'none' }}
                >
          ...
          <div className="modal-body">
            ...
            {mode === 'ai' ? (
              <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '8px 0 20px' }}>
                      <button
                        ...
                        style={{
                          ...
                          boxShadow: isRecording ? '0 0 0 8px rgba(244,63,94,0.12)' : 'none',
                          marginBottom: 12,
                        }}
                      >
                        {isRecording
                          ? <WaveformBars />
                          : <span className="material-symbols-outlined" style={{ fontSize: 36 }}>mic_none</span>
                        }
                      </button>
                      ...
                      {isRecording ? (
                        <>
                          <div style={{ fontVariantNumeric: 'tabular-nums', fontSize: 28, fontWeight: 800, color: 'var(--red)', letterSpacing: 2 }}>
                            {formatSeconds(recordSeconds)}
                          </div>
                          <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 4 }}>
                            Berhenti otomatis setelah 2 detik diam
                          </div>
                        </>
                      ) : (
                        <>
                          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)' }}>
                            {aiText ? 'Tap untuk rekam lagi' : 'Tap untuk mulai merekam'}
                          </div>
                          <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>
                            Bahasa Indonesia · Auto-stop saat diam
                          </div>
                        </>
                      )}
                    </div>
                ...
                {/* ── PANEL: Kamera / Galeri (shared image panel) ── */}
                  ...
                  <div style={{ animation: 'fadeIn 0.2s ease' }}>
                    {imagePreview ? (
                      <>
                        ...
                        <div style={{ position: 'relative', marginBottom: 14 }}>
                          <img
                            src={imagePreview}
                            alt="Receipt Preview"
                            style={{ width: '100%', maxHeight: 200, objectFit: 'contain', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--surface2)' }}
                          />
                          <button
                            type="button"
                        ...
                        <div className="form-group">
                          ...
                          <input
                            className="form-input"
                            type="text"
                            placeholder="Misal: oleh aldin, kemarin"
                            value={aiText}
                            onChange={e => setAiText(e.target.value)}
                          />
                        </div>
                      ...
                      <div style={{ textAlign: 'center', padding: '32px 16px' }}>
                        <span className="material-symbols-outlined" style={{ fontSize: 48, color: 'var(--text3)', display: 'block', marginBottom: 12 }}>
                        ...
                        <button
                          ...
                          onClick={() => document.getElementById(aiInputMode === 'camera' ? 'receipt-upload-camera' : 'receipt-upload-gallery').click()}
                        >
