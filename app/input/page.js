...
"expense"  → pengeluaran / belanja / bayar sesuatu (bukan tarik tunai)
"income"   → pemasukan / gaji / transfer masuk / dapat uang
"cash"     → KHUSUS tarik tunai / ambil uang ATM
"transfer" → pindah dana antar rekening atau antar pengguna:
  Kata kunci: "transfer ke", "kirim ke", "kasih", "pindahin", "geser", "bayarin", "kasih duit ke", "transfer dari", "bayar ke"

CONTOH:
- "transfer 500rb ke solikhatun" → tipe: "transfer"
- "kirim 200rb ke aldin BCA" → tipe: "transfer"
- "pindahin 300rb dari BCA ke Mandiri" → tipe: "transfer"
- "tarik tunai 500rb BCA" → tipe: "cash"
- "beli bensin 50rb" → tipe: "expense"
- "gaji 5jt" → tipe: "income"

=== USER ===
Daftar user: ${JSON.stringify(userList)}
...
- Kembalikan HANYA JSON tanpa markdown`
      ...
      if (result.tipe === 'transfer') {
        ...
        // Fallback user matching via prefix jika AI tidak bisa mapping
  ...
  // ── Simpan transfer dari confirm popup ──
  ...
  // ── Confirm popup (expense/income/cash) ──
        ...
        {mode === 'ai' && (
          <div className="card">
            ...
            <div style={{ background: 'var(--surface2)', borderRadius: 10, padding: '10px 12px', marginBottom: 14, fontSize: 12, color: 'var(--text3)', lineHeight: 1.6 }}>
              ...
              <div>• <span style={{ color: 'var(--text1)' }}>Expense:</span> "beli bensin 50rb aldin BCA"</div>
              <div>• <span style={{ color: 'var(--text1)' }}>Income:</span> "gaji 5jt mandiri aldin"</div>
              <div>• <span style={{ color: 'var(--text1)' }}>Tarik tunai:</span> "tarik 200rb ATM BCA"</div>
              <div>• <span style={{ color: 'var(--accent)', fontWeight: 700 }}>Transfer:</span> "transfer 500rb ke solikhatun BCA dari aldin"</div>
