...
export default function LoginPage() {
  ...
  const [bioUser, setBioUser]             = useState(null) // user setelah login email sukses
  ...
  useEffect(() => {
    async function checkBio() {
      ...
      setBioRegistered(supported && isBiometricRegistered())
    }
    checkBio()
  }, [])
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      // Cek apakah perlu tawarkan register biometrik
      if (bioSupported && !isBiometricRegistered() && user) {
        setBioUser(user)
  ...
  async function handleBiometricLogin() {
    ...
    try {
      ...
      // Jika session habis, minta login email dulu
      if (err.message.includes('Session habis')) {
        setError('Sesi habis. Silakan login dengan email & password terlebih dahulu.')
        // Hapus credential lama supaya tidak loop
        removeBiometricCred()
  ...
  async function handleRegisterBio() {
    ...
    setBioLoading(true)
    try {
      await registerBiometric(bioUser.id, bioUser.email?.split('@')[0] || 'User')
      setBioRegistered(true)
      router.push('/dashboard')
    } catch (err) {
      if (err.name === 'NotAllowedError') {
        // User cancel — lanjut ke dashboard tanpa bio
        router.push('/dashboard')
      } else {
  ...
  // ── Popup tawaran register biometrik ──
  if (showRegisterBio) {
        ...
        <div style={{ width: '100%', maxWidth: 360 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 28, boxShadow: '0 4px 12px rgba(0,0,0,0.06)', textAlign: 'center' }}>
            <div style={{ fontSize: 52, marginBottom: 12 }}>
              {/iPhone|iPad|Mac/.test(typeof navigator !== 'undefined' ? navigator.userAgent : '') ? '🔒' : '🫆'}
            </div>
            ...
            <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 24, lineHeight: 1.6 }}>
              Login lebih cepat lain kali menggunakan Face ID, Touch ID, atau sidik jari tanpa perlu ketik password.
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <button
                onClick={handleRegisterBio}
                disabled={bioLoading}
                style={{ width: '100%', height: 48, fontSize: 15, fontWeight: 700, background: 'var(--accent)', color: 'white', border: 'none', borderRadius: 10, cursor: bioLoading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: bioLoading ? 0.8 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
              >
                {bioLoading ? 'Memproses...' : '✅ Ya, Aktifkan'}
              </button>
              <button
                onClick={() => router.push('/dashboard')}
                style={{ width: '100%', height: 44, fontSize: 14, fontWeight: 600, background: 'var(--surface2)', color: 'var(--text2)', border: '1px solid var(--border)', borderRadius: 10, cursor: 'pointer', fontFamily: 'inherit' }}
              >
                Nanti Saja
              </button>
            </div>
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', background: 'var(--bg)' }}>
      ...
      {/* ── BIOMETRIC BUTTON (muncul kalau sudah terdaftar) ── */}
      {bioSupported && bioRegistered && !showForgot && (
        <div style={{ width: '100%', maxWidth: 360, marginBottom: 16 }}>
          ...
          >
            {bioLoading ? (
              <>
                <span className="material-symbols-outlined" style={{ fontSize: 22, animation: 'spin 0.8s linear infinite' }}>refresh</span>
                Verifikasi...
              </>
            ) : (
              <>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, boxShadow: '0 4px 12px rgba(0,0,0,0.06)' }}>
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <button type="button" onClick={() => setShowPass(!showPass)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', padding: 4 }}>
            ...
            <button type="submit" disabled={loading} style={{ marginTop: 20, width: '100%', height: 46, fontSize: 15, fontWeight: 700, background: 'var(--accent)', color: 'white', border: 'none', borderRadius: 8, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'inherit', opacity: loading ? 0.8 : 1 }}>
              {loading ? (
                <>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 24, boxShadow: '0 4px 12px rgba(0,0,0,0.06)' }}>
            ...
            {forgotMsg ? (
              ...
              <form onSubmit={handleForgot}>
                ...
                <button type="submit" disabled={forgotLoading} style={{ width: '100%', height: 44, fontSize: 14, fontWeight: 700, background: 'var(--accent)', color: 'white', border: 'none', borderRadius: 8, cursor: forgotLoading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'inherit', opacity: forgotLoading ? 0.8 : 1 }}>
