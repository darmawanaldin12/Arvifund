...
import { fmt, fmtFull, fmtTanggalShort, BULAN_ORDER, KATEGORI_ICON, KATEGORI_COLOR, getMoMInfo, parseTanggal } from '../../lib/utils'
              ...
              <div style={{ display: 'flex', gap: 3, alignItems: 'flex-end', height: 48 }}>
                {[0,1,2,3,4,5,6].map(d => {
                  ...
                  const maxDay = Math.max(...[0,1,2,3,4,5,6].map(x => {
                    ...
                    return filteredExpenses.filter(r => r.tanggal?.startsWith(dx.toISOString().split('T')[0])).reduce((s,r) => s+r.nilai, 0)
                  }), 1)
